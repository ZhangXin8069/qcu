...
bool if_converge(void* b_vec, void* x_vec, void* res_vec, void* partial_vec, void* gauge, QcuParam *param) {
  ...
  // D dagger D x = b?
  MmV_one_round (res_vec, x_vec, gauge, param);// Ap, A = d dagger d
...
#endif
{
  ...
  MmV_one_round (temp_vec1, p_vec, gauge, param); // Ap--->temp_vec1
...
// #ifdef DEBUG
//   printf(RED"line %d, difference between p and new p: %lf \n", __LINE__, compare_two_vectors_cpu (debug_ptr, x_vec, vol));
//   printf(CLR"");
// #endif
  ...
  // r' <- r'- \alpha A p
// #ifdef DEBUG
//   printf("before %.23lf %.23lf\n", alpha.real(), alpha.imag());
// #endif
//   alpha = alpha * Complex(-1, 0);    // alpha = -alpha
// #ifdef DEBUG
//   printf("after %.23lf %.23lf\n", alpha.real(), alpha.imag());
// #endif
...
// #ifdef DEBUG
//   printf(RED"line %d, difference between r' and new r: %lf \n", __LINE__, compare_two_vectors_cpu (temp_vec2, r_vec, vol));
//   printf(CLR"");
// #endif

  // : if converge, return x
  //if (if_converge(b_vec, x_vec, res_vec, partial_vec, gauge, param)) {
...
// #ifdef DEBUG
//   // printf(RED"denominator %lf, %lf\n", denominator.real(), denominator.imag());
//   // printf(RED"numerator %lf, %lf\n", numerator.real(), numerator.imag());
//   printf(RED"beta %lf, %lf\n", beta.real(), beta.imag());
//   printf(CLR"");
//   // printf("line %d, difference between p and x: %lf \n", __LINE__, compare_two_vectors_cpu (p_vec, x_vec, vol));
// #endif

// #ifdef DEBUG
//   checkCudaErrors(cudaMemcpy(debug_ptr, p_vec, sizeof(Complex) * Ns * Nc * vol, cudaMemcpyDeviceToDevice));
// #endif
// checkCudaErrors(cudaMemcpy(debug_ptr, p_vec, sizeof(Complex) * Ns * Nc * vol, cudaMemcpyDeviceToDevice));
  gpu_sclar_multiply_vector (p_vec, d_beta, vol); // p_vec = \beta p_vec

// #ifdef DEBUG
//   printf(RED"line %d, difference between p and new p: %lf \n", __LINE__, compare_two_vectors_cpu (debug_ptr, p_vec, vol));
//   printf(CLR"");
// #endif
...
// #endif
// #ifdef DEBUG
//   mpi_comm->interprocess_inner_prod_barrier(temp_vec2, temp_vec2, d_numerator, vol);  // <r, r> --> d_numerator
//   checkCudaErrors(cudaMemcpy(&numerator, d_numerator, sizeof(Complex), cudaMemcpyDeviceToHost));
//   printf(BLUE"numerator %lf, %lf\n", numerator.real(), numerator.imag());
//   printf(CLR"");
// #endif
  one = Complex(1, 0);
...
// #ifdef DEBUG
//   mpi_comm->interprocess_inner_prod_barrier(p_vec, p_vec, d_denominator, vol);  // <r', r'>
//   cudaMemcpy(&denominator, d_denominator, sizeof(Complex) * vol * Ns * Nc, cudaMemcpyDeviceToHost);
//   printf(BLUE"p inner prod: %lf %lf", denominator.real(), denominator.imag());
//   printf(CLR"");
// #endif
// #ifdef DEBUG
//   printf(RED"line %d, difference between p = r'+beta p and new beta p: %lf \n", __LINE__, compare_two_vectors_cpu (debug_ptr, p_vec, vol));
//   printf(CLR"");
// #endif
...
void cg_inverter(void* b_vector, void* x_vector, void *gauge, QcuParam *param) {
  ...
  //  if converge, return x
  // if (if_converge(d_new_b, x_vector, res_vec, partial_result_vector, gauge, param)) {
  if (if_converge(r_vec, vol)) {
    // if_end = true;
    printf("cg success!!!\n");
