...
#  cp.cuda.runtime.deviceSynchronize()
  ...
  '''
  print(f'========norm of Mp: {cp.linalg.norm(Mp.data)}')
  qcu.dslashQcu(Mp1.even_ptr, p.even_ptr, U.data_ptr, param, 1)
  print(f'rank {rank} difference: , {cp.linalg.norm(Mp1.data - Mp.data)}')

  print(f'rank {rank} my x and x difference: , {cp.linalg.norm(p.data - Mp1.data) / cp.linalg.norm(p.data)}')
  #qcu.cg_inverter(p.even_ptr, Mp1.even_ptr, U.data_ptr, param)
