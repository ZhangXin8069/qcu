...
def test_mpi(round):
    ...
    if rank == 0:
        print('===============qcu==================')
    t1 = perf_counter()
    qcu.cg_inverter(x_vector.even_ptr, p.even_ptr, U.data_ptr,
                    param, 1e-9, 0.125)  # Dslash x_vector = Mp1, get x_vector
    cp.cuda.runtime.deviceSynchronize()
    t2 = perf_counter()
    quda.MatQuda(Mp1.data_ptr, x_vector.data_ptr, dslash.invert_param)

    print(f'rank {rank} my x and x difference: , {cp.linalg.norm(Mp1.data - p.data) / cp.linalg.norm(Mp1.data)}, takes {t2 - t1} sec, my_x_norm = {cp.linalg.norm(x_vector.data)}')
    print(f'qcu rank {rank} takes {t2 - t1} sec')

    # print(f'my x res = {x_vector.data[1, 0, 0, 0, 0]}')
    # print(f'my even_x res norm = {cp.linalg.norm(x_vector.data[0])}')
    # print(f'my odd_x res norm = {cp.linalg.norm(x_vector.data[1])}')
    # print(f'quda even_x res norm = {cp.linalg.norm(quda_x.data[0])}')
    # print(f'quda odd_x res norm = {cp.linalg.norm(quda_x.data[1])}')
    # print(f'even diff = {cp.linalg.norm(quda_x.data[0] - x_vector.data[0]) / cp.linalg.norm(quda_x.data[0])}')
    # print(f'odd diff = {cp.linalg.norm(quda_x.data[1] - x_vector.data[1]) / cp.linalg.norm(quda_x.data[1])}')
