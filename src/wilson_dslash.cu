...
__global__ void wilson_dslash(void *device_U, void *device_src,
  ...
  int lat_tzyxcc = lat_tzyx * _LAT_CC_;
  int move;
  ...
  tmp_U = (origin_U + move + (1 - parity) * lat_tzyxcc);
...
tmp_U = (origin_U + move * lat_x + (3 - parity) * lat_tzyxcc);
give_u(U, tmp_U, lat_tzyx);
...
tmp_U = (origin_U + move * lat_y * lat_x + (5 - parity) * lat_tzyxcc);
give_u(U, tmp_U, lat_tzyx);
...
{ // t part
  {
    ...
    tmp_U =
        (origin_U + move * lat_z * lat_y * lat_x + (7 - parity) * lat_tzyxcc);
    give_u(U, tmp_U, lat_tzyx);
...
__global__ void wilson_dslash_x_send(void *device_U, void *device_src,
  ...
  { // x-1
    ...
    { // sigma src
      ...
      give_send_x(origin_b_x_send_vec, b_x_send_vec, lat_tzyx, (move == 0));
    }
  }
  {
    x = lat_x - 1; // f_x
  ...
  { // x+1
    ...
    { // just tmp
      ...
      give_send_x(origin_f_x_send_vec, f_x_send_vec, lat_tzyx, (move == 0));
    }
  }
#endif
}
__global__ void wilson_dslash_x_recv(void *device_U, void *device_dest,
  ...
  { // x-1
    move_backward_x(move, x, lat_x, eo, parity);
    // recv in x-1 way
    give_recv(b_x_recv_vec, origin_b_x_recv_vec, lat_tzyx);
    for (int c0 = 0; c0 < _LAT_C_; c0++) {
  ...
  { // x+1
    move_forward_x(move, x, lat_x, eo, parity);
    // recv in x+1 way
    give_recv(f_x_recv_vec, origin_f_x_recv_vec, lat_tzyx);
    tmp_U = (origin_U + parity * lat_tzyxcc);
    give_u(U, tmp_U, lat_tzyx);
    {
      for (int c0 = 0; c0 < _LAT_C_; c0++) {
...
__global__ void wilson_dslash_y_send(void *device_U, void *device_src,
  ...
  { // y-1
    ...
    { // sigma src
      ...
      give_send(origin_b_y_send_vec, b_y_send_vec, lat_tzyx);
    }
  }
  {
    y = lat_y - 1; // f_y
  ...
  { // y+1
    ...
    { // just tmp
      ...
      give_send(origin_f_y_send_vec, f_y_send_vec, lat_tzyx);
    }
  }
#endif
}
__global__ void wilson_dslash_y_recv(void *device_U, void *device_dest,
  ...
  { // y-1
    move_backward(move, y, lat_y);
    // recv in y-1 way
    give_recv(b_y_recv_vec, origin_b_y_recv_vec, lat_tzyx);
    for (int c0 = 0; c0 < _LAT_C_; c0++) {
  ...
  { // y+1
    // move_forward(move, y, lat_y);
    // recv in y+1 way
    give_recv(f_y_recv_vec, origin_f_y_recv_vec, lat_tzyx);
    tmp_U = (origin_U + (2 + parity) * lat_tzyxcc);
    give_u(U, tmp_U, lat_tzyx);
    {
      for (int c0 = 0; c0 < _LAT_C_; c0++) {
...
__global__ void wilson_dslash_z_send(void *device_U, void *device_src,
  ...
  { // z-1
    ...
    { // sigma src
      ...
      give_send(origin_b_z_send_vec, b_z_send_vec, lat_tzyx);
    }
  }
  {
    z = lat_z - 1; // f_z
  ...
  { // z+1
    ...
    { // just tmp
      ...
      give_send(origin_f_z_send_vec, f_z_send_vec, lat_tzyx);
    }
  }
#endif
}
__global__ void wilson_dslash_z_recv(void *device_U, void *device_dest,
  ...
  { // z-1
    // move_backward(move, z, lat_z);
    // recv in z-1 way
    give_recv(b_z_recv_vec, origin_b_z_recv_vec, lat_tzyx);
    for (int c0 = 0; c0 < _LAT_C_; c0++) {
  ...
  { // z+1
    // move_forward(move, z, lat_z);
    // recv in z+1 way
    give_recv(f_z_recv_vec, origin_f_z_recv_vec, lat_tzyx);
    tmp_U = (origin_U + (4 + parity) * lat_tzyxcc);
    give_u(U, tmp_U, lat_tzyx);
    {
      for (int c0 = 0; c0 < _LAT_C_; c0++) {
...
__global__ void wilson_dslash_t_send(void *device_U, void *device_src,
  ...
  { // t-1
    ...
    { // sigma src
      ...
      give_send(origin_b_t_send_vec, b_t_send_vec, lat_tzyx);
    }
  }
  {
    t = lat_t - 1; // f_t
  ...
  { // t+1
    ...
    { // just tmp
      ...
      give_send(origin_f_t_send_vec, f_t_send_vec, lat_tzyx);
    }
  }
#endif
}
__global__ void wilson_dslash_t_recv(void *device_U, void *device_dest,
  ...
  { // t-1
    // move_backward(move, t, lat_t);
    // recv in t-1 way
    give_recv(b_t_recv_vec, origin_b_t_recv_vec, lat_tzyx);
    for (int c0 = 0; c0 < _LAT_C_; c0++) {
  ...
  { // t+1
    // move_forward(move, t, lat_t);
    // recv in t+1 way
    give_recv(f_t_recv_vec, origin_f_t_recv_vec, lat_tzyx);
    tmp_U = (origin_U + (6 + parity) * lat_tzyxcc);
    give_u(U, tmp_U, lat_tzyx);
    {
      for (int c0 = 0; c0 < _LAT_C_; c0++) {
