  ...
  dim3 blockDim(BLOCK_SIZE);
  {
    ...
    // mpi_wilson_dslash
    {
      ...
      if (grid_1dim[X] != 1)
      {
        ...
        MPI_Irecv(recv_vec[B_X], lat_3dim12[YZT], MPI_DOUBLE, move_b, 1,
                  MPI_COMM_WORLD, &recv_request[B_X]);
        MPI_Irecv(recv_vec[F_X], lat_3dim12[YZT], MPI_DOUBLE, move_f, 0,
                  MPI_COMM_WORLD, &recv_request[F_X]);
        MPI_Isend(send_vec[B_X], lat_3dim12[YZT], MPI_DOUBLE, move_b, 0,
                  MPI_COMM_WORLD, &send_request[B_X]);
        MPI_Isend(send_vec[F_X], lat_3dim12[YZT], MPI_DOUBLE, move_f, 1,
      ...
      if (grid_1dim[Y] != 1)
      {
        ...
        MPI_Irecv(recv_vec[B_Y], lat_3dim12[XZT], MPI_DOUBLE, move_b, 3,
                  MPI_COMM_WORLD, &recv_request[B_Y]);
        MPI_Irecv(recv_vec[F_Y], lat_3dim12[XZT], MPI_DOUBLE, move_f, 2,
                  MPI_COMM_WORLD, &recv_request[F_Y]);
        MPI_Isend(send_vec[B_Y], lat_3dim12[XZT], MPI_DOUBLE, move_b, 2,
                  MPI_COMM_WORLD, &send_request[B_Y]);
        MPI_Isend(send_vec[F_Y], lat_3dim12[XZT], MPI_DOUBLE, move_f, 3,
      ...
      if (grid_1dim[Z] != 1)
      {
        ...
        MPI_Irecv(recv_vec[B_Z], lat_3dim12[XYT], MPI_DOUBLE, move_b, 5,
                  MPI_COMM_WORLD, &recv_request[B_Z]);
        MPI_Irecv(recv_vec[F_Z], lat_3dim12[XYT], MPI_DOUBLE, move_f, 4,
                  MPI_COMM_WORLD, &recv_request[F_Z]);
        MPI_Isend(send_vec[B_Z], lat_3dim12[XYT], MPI_DOUBLE, move_b, 4,
                  MPI_COMM_WORLD, &send_request[B_Z]);
        MPI_Isend(send_vec[F_Z], lat_3dim12[XYT], MPI_DOUBLE, move_f, 5,
      ...
      if (grid_1dim[T] != 1)
      {
        ...
        MPI_Irecv(recv_vec[B_T], lat_3dim12[XYZ], MPI_DOUBLE, move_b, 7,
                  MPI_COMM_WORLD, &recv_request[B_T]);
        MPI_Irecv(recv_vec[F_T], lat_3dim12[XYZ], MPI_DOUBLE, move_f, 6,
                  MPI_COMM_WORLD, &recv_request[F_T]);
        MPI_Isend(send_vec[B_T], lat_3dim12[XYZ], MPI_DOUBLE, move_b, 6,
                  MPI_COMM_WORLD, &send_request[B_T]);
        MPI_Isend(send_vec[F_T], lat_3dim12[XYZ], MPI_DOUBLE, move_f, 7,
                  MPI_COMM_WORLD, &send_request[F_T]);
      }
      // recv x
      if (grid_1dim[X] != 1)
      ...
      // mpi_wilson_dslash
      {
        ...
        if (grid_1dim[X] != 1)
        {
          ...
          wilson_dslash_x_recv<<<gridDim, blockDim>>>(
              gauge, cg_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T], parity, recv_vec[B_X],
              recv_vec[F_X]);
        }
        else
        {
          checkCudaErrors(cudaDeviceSynchronize());
          wilson_dslash_x_recv<<<gridDim, blockDim>>>(
              gauge, cg_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T], parity, send_vec[F_X],
              send_vec[B_X]);
        }
        // recv y
        if (grid_1dim[Y] != 1)
        {
          ...
          wilson_dslash_y_recv<<<gridDim, blockDim>>>(
              gauge, cg_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T], parity, recv_vec[B_Y],
              recv_vec[F_Y]);
        }
        else
        {
          checkCudaErrors(cudaDeviceSynchronize());
          wilson_dslash_y_recv<<<gridDim, blockDim>>>(
              gauge, cg_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T], parity, send_vec[F_Y],
              send_vec[B_Y]);
        }
        // recv z
        if (grid_1dim[Z] != 1)
        {
          ...
          wilson_dslash_z_recv<<<gridDim, blockDim>>>(
              gauge, cg_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T], parity, recv_vec[B_Z],
              recv_vec[F_Z]);
        }
        else
        {
          checkCudaErrors(cudaDeviceSynchronize());
          wilson_dslash_z_recv<<<gridDim, blockDim>>>(
              gauge, cg_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T], parity, send_vec[F_Z],
              send_vec[B_Z]);
        }
        // recv t
        if (grid_1dim[T] != 1)
        {
          ...
          wilson_dslash_t_recv<<<gridDim, blockDim>>>(
              gauge, cg_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T], parity, recv_vec[B_T],
              recv_vec[F_T]);
        }
        else
        {
          checkCudaErrors(cudaDeviceSynchronize());
          wilson_dslash_t_recv<<<gridDim, blockDim>>>(
              gauge, cg_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T], parity, send_vec[F_T],
              send_vec[B_T]);
        }
        MPI_Barrier(MPI_COMM_WORLD);
      }
