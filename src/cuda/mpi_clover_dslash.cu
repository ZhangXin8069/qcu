...
__global__ void mpi_inverse_clover(void *device_clover, int device_lat_x,
...
__global__ void mpi_give_clover(void *device_clover, void *device_dest,
                                int device_lat_x, const int device_lat_y,
                                const int device_lat_z) {
  LatticeComplex *origin_clover;
...
void mpiDslashCloverQcu(void *fermion_out, void *fermion_in, void *gauge,
  ...
  }
  {
    ...
    checkCudaErrors(cudaDeviceSynchronize());
    auto start = std::chrono::high_resolution_clock::now();
    mpi_make_clover<<<gridDim, blockDim>>>(gauge, clover, lat_x, lat_y, lat_z,
                                       lat_t, parity);
    checkCudaErrors(cudaDeviceSynchronize());
    auto end = std::chrono::high_resolution_clock::now();
    auto duration =
  ...
  }
  {
    ...
    checkCudaErrors(cudaDeviceSynchronize());
    auto start = std::chrono::high_resolution_clock::now();
    mpi_inverse_clover<<<gridDim, blockDim>>>(clover, lat_x, lat_y, lat_z);
    checkCudaErrors(cudaDeviceSynchronize());
    auto end = std::chrono::high_resolution_clock::now();
    auto duration =
        std::chrono::duration_cast<std::chrono::nanoseconds>(end - start)
  ...
  }
  {
    ...
    checkCudaErrors(cudaDeviceSynchronize());
    auto start = std::chrono::high_resolution_clock::now();
    mpi_give_clover<<<gridDim, blockDim>>>(clover, fermion_out, lat_x, lat_y,
                                       lat_z);
    checkCudaErrors(cudaDeviceSynchronize());
    auto end = std::chrono::high_resolution_clock::now();
    auto duration =
