...
void testDslashQcu(void *fermion_out, void *fermion_in, void *gauge,
  ...
  const int lat_t = param->lattice_size[3];
  //void *clover;
  //checkCudaErrors(cudaMalloc(&clover, (lat_t * lat_z * lat_y * lat_x * 144) * sizeof(LatticeComplex)));
  cudaError_t err;
  dim3 gridDim(lat_x * lat_y * lat_z * lat_t / BLOCK_SIZE);
  dim3 blockDim(BLOCK_SIZE);
  {
    ...
    checkCudaErrors(cudaDeviceSynchronize());
    auto start = std::chrono::high_resolution_clock::now();
    test_wilson_dslash<<<gridDim, blockDim>>>(gauge, fermion_in, fermion_out, lat_x,
                                         lat_y, lat_z, lat_t, parity);
    err = cudaGetLastError();
    checkCudaErrors(err);
    checkCudaErrors(cudaDeviceSynchronize());
    ...
    auto duration =
        std::chrono::duration_cast<std::chrono::nanoseconds>(end - start)
            .count();
    printf(
        "test wilson dslash total time: (without malloc free memcpy) : %.9lf sec\n",
        double(duration) / 1e9);
  }
  // {
  //   // make clover
...
void mpiDslashQcu(void *fermion_out, void *fermion_in, void *gauge,
  ...
  dim3 blockDim(BLOCK_SIZE);
  {
    ...
    if (grid_x != 1) {
      ...
      wilson_dslash_x_recv<<<gridDim, blockDim>>>(gauge, fermion_out, lat_x,
                                                  lat_y, lat_z, lat_t, parity,
                                                  f_x_send_vec, b_x_send_vec);//debug
    }
    // recv y
    if (grid_y != 1) {
