...
__global__ void wilson_dslash_clear_dest(void *device_dest, int *lat_1dim) {
  int parity = blockIdx.x * blockDim.x + threadIdx.x;
  int move;
  move = lat_1dim[X] * lat_1dim[Y] * lat_1dim[Z];
  const int t = parity / move;
  parity -= t * move;
  move = lat_1dim[X] * lat_1dim[Y];
  const int z = parity / move;
  parity -= z * move;
  const int y = parity / lat_1dim[X];
  const int x = parity - y * lat_1dim[X];
  LatticeComplex zero(0.0, 0.0);
  LatticeComplex *origin_dest =
      ((static_cast<LatticeComplex *>(device_dest)) +
       t * lat_1dim[Z] * lat_1dim[Y] * lat_1dim[X] * 12 +
       z * lat_1dim[Y] * lat_1dim[X] * 12 + y * lat_1dim[X] * 12 + x * 12);
  give_value(origin_dest, zero, 12);
}
__global__ void wilson_dslash_x_send(void *device_U, void *device_src,
                                     void *device_dest, int *lat_1dim,
                                     const int device_parity,
                                     void *device_b_x_send_vec,
                                     void *device_f_x_send_vec) {
  int parity = blockIdx.x * blockDim.x + threadIdx.x;
  const int lat_xcc = lat_1dim[X] * 9;
  const int lat_yxcc = lat_1dim[Y] * lat_xcc;
  const int lat_zyxcc = lat_1dim[Z] * lat_yxcc;
  const int lat_tzyxcc = lat_1dim[T] * lat_zyxcc;
  const int lat_xsc = lat_1dim[X] * 12;
  const int lat_yxsc = lat_1dim[Y] * lat_xsc;
  const int lat_zyxsc = lat_1dim[Z] * lat_yxsc;
  int move;
  move = lat_1dim[X] * lat_1dim[Y] * lat_1dim[Z];
  const int t = parity / move;
  parity -= t * move;
  ...
  LatticeComplex *origin_b_x_send_vec =
      ((static_cast<LatticeComplex *>(device_b_x_send_vec)) +
       (t * lat_1dim[Z] * lat_1dim[Y] + z * lat_1dim[Y] + y) * 6);
  ...
  {
    // x-1
    move_backward_x(move, x, lat_1dim[X], eo, parity);
    if (move != lat_1dim[X] - 1) {
  ...
  {
    // x+1
    move_forward_x(move, x, lat_1dim[X], eo, parity);
    if (move != 1 - lat_1dim[X]) {
    ...
    if (x == lat_1dim[X] - 1 && move == 0) { // even-odd
      // send in x-1 way
...
__global__ void wilson_dslash_x_recv(void *device_U, void *device_dest,
                                     int *lat_1dim, const int device_parity,
  ...
  LatticeComplex *origin_U =
      ((static_cast<LatticeComplex *>(device_U)) +
       t * lat_1dim[Z] * lat_1dim[Y] * lat_1dim[X] * 9 +
       z * lat_1dim[Y] * lat_1dim[X] * 9 + y * lat_1dim[X] * 9 + x * 9);
  LatticeComplex *origin_dest =
      ((static_cast<LatticeComplex *>(device_dest)) +
       t * lat_1dim[Z] * lat_1dim[Y] * lat_1dim[X] * 12 +
       z * lat_1dim[Y] * lat_1dim[X] * 12 + y * lat_1dim[X] * 12 + x * 12);
  ...
  }
  {
    ...
    if (move == 1 - lat_1dim[X]) {
      ...
      tmp_U = (origin_U + parity * lat_1dim[T] * lat_1dim[Z] * lat_1dim[Y] *
                              lat_1dim[X] * 9);
      give_u(U, tmp_U);
      {
        for (int c0 = 0; c0 < 3; c0++) {
...
__global__ void wilson_dslash_y_send(void *device_U, void *device_src,
  ...
  LatticeComplex *origin_b_y_send_vec =
      ((static_cast<LatticeComplex *>(device_b_y_send_vec)) +
       (t * lat_1dim[Z] * lat_1dim[X] + z * lat_1dim[X] + x) * 6);
  ...
  {
    // y-1
    move_backward(move, y, lat_1dim[Y]);
    if (move == -1) {
  ...
  {
    // y+1
    move_forward(move, y, lat_1dim[Y]);
    if (move == 1) {
...
__global__ void wilson_dslash_y_recv(void *device_U, void *device_dest,
  ...
  }
  {
    ...
    if (move != 1) {
      ...
      tmp_U = (origin_U + (parity + 2) * lat_1dim[T] * lat_1dim[Z] *
                              lat_1dim[Y] * lat_1dim[X] * 9);
      give_u(U, tmp_U);
      {
        for (int c0 = 0; c0 < 3; c0++) {
          tmp0 = zero;
...
__global__ void wilson_dslash_z_send(void *device_U, void *device_src,
  ...
  LatticeComplex *origin_b_z_send_vec =
      ((static_cast<LatticeComplex *>(device_b_z_send_vec)) +
       (t * lat_1dim[Y] * lat_1dim[X] + y * lat_1dim[X] + x) * 6);
  ...
  {
    // z-1
    move_backward(move, z, lat_1dim[Z]);
    if (move == -1) {
  ...
  {
    // z+1
    move_forward(move, z, lat_1dim[Z]);
    if (move == 1) {
...
__global__ void wilson_dslash_z_recv(void *device_U, void *device_dest,
  ...
  }
  {
    ...
    if (move != 1) {
      ...
      tmp_U = (origin_U + (parity + 4) * lat_1dim[T] * lat_1dim[Z] *
                              lat_1dim[Y] * lat_1dim[X] * 9);
      give_u(U, tmp_U);
      {
        for (int c0 = 0; c0 < 3; c0++) {
...
__global__ void wilson_dslash_t_send(void *device_U, void *device_src,
  ...
  LatticeComplex *origin_b_t_send_vec =
      ((static_cast<LatticeComplex *>(device_b_t_send_vec)) +
       (z * lat_1dim[Y] * lat_1dim[X] + y * lat_1dim[X] + x) * 6);
  ...
  {
    // t-1
    move_backward(move, t, lat_1dim[T]);
    if (move == -1) {
  ...
  {
    // t+1
    move_forward(move, t, lat_1dim[T]);
    if (move == 1) {
...
__global__ void wilson_dslash_t_recv(void *device_U, void *device_dest,
  ...
  }
  {
    ...
    if (move != 1) {
      ...
      tmp_U = (origin_U + (parity + 6) * lat_1dim[T] * lat_1dim[Z] *
                              lat_1dim[Y] * lat_1dim[X] * 9);
      give_u(U, tmp_U);
      {
        for (int c0 = 0; c0 < 3; c0++) {
...
void mpiDslashQcu(void *fermion_out, void *fermion_in, void *gauge,
  ...
  void *recv_vec[WARDS];
  for (int i = 0; i < DIM; i++) {
    cudaMallocManaged(&send_vec[i * SR], lat_3dim6[i] * sizeof(LatticeComplex));
    cudaMallocManaged(&send_vec[i * SR + 1],
                      lat_3dim6[i] * sizeof(LatticeComplex));
    cudaMallocManaged(&recv_vec[i * SR], lat_3dim6[i] * sizeof(LatticeComplex));
  ...
  // clean
  wilson_dslash_clear_dest<<<gridDim, blockDim>>>(fermion_out, lat_1dim);
  // send x
  wilson_dslash_x_send<<<gridDim, blockDim>>>(gauge, fermion_in, fermion_out,
                                              lat_1dim, parity, send_vec[B_X],
                                              send_vec[F_X]);
  if (grid_1dim[X] != 1) {
  ...
  // send y
  wilson_dslash_y_send<<<gridDim, blockDim>>>(gauge, fermion_in, fermion_out,
                                              lat_1dim, parity, send_vec[B_Y],
                                              send_vec[F_Y]);
  if (grid_1dim[Y] != 1) {
    checkCudaErrors(cudaDeviceSynchronize());
  ...
  // send z
  wilson_dslash_z_send<<<gridDim, blockDim>>>(gauge, fermion_in, fermion_out,
                                              lat_1dim, parity, send_vec[B_Z],
                                              send_vec[F_Z]);
  if (grid_1dim[Z] != 1) {
    checkCudaErrors(cudaDeviceSynchronize());
  ...
  // send t
  wilson_dslash_t_send<<<gridDim, blockDim>>>(gauge, fermion_in, fermion_out,
                                              lat_1dim, parity, send_vec[B_T],
                                              send_vec[F_T]);
  if (grid_1dim[T] != 1) {
    checkCudaErrors(cudaDeviceSynchronize());
  ...
  if (grid_1dim[X] != 1) {
    ...
    wilson_dslash_x_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim, parity, recv_vec[B_X], recv_vec[F_X]);
  } else {
    checkCudaErrors(cudaDeviceSynchronize());
    wilson_dslash_x_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim, parity, send_vec[F_X], send_vec[B_X]);
  }
  // recv y
  if (grid_1dim[Y] != 1) {
    ...
    wilson_dslash_y_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim, parity, recv_vec[B_Y], recv_vec[F_Y]);
  } else {
    checkCudaErrors(cudaDeviceSynchronize());
    wilson_dslash_y_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim, parity, send_vec[F_Y], send_vec[B_Y]);
  }
  // recv z
  if (grid_1dim[Z] != 1) {
    ...
    wilson_dslash_z_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim, parity, recv_vec[B_Z], recv_vec[F_Z]);
  } else {
    checkCudaErrors(cudaDeviceSynchronize());
    wilson_dslash_z_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim, parity, send_vec[F_Z], send_vec[B_Z]);
  }
  // recv t
  if (grid_1dim[T] != 1) {
    ...
    wilson_dslash_t_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim, parity, recv_vec[B_T], recv_vec[F_T]);
  } else {
    checkCudaErrors(cudaDeviceSynchronize());
    wilson_dslash_t_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim, parity, send_vec[F_T], send_vec[B_T]);
  }
