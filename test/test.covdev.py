...
def covdev(U: LatticeGauge, x: LatticeFermion, mu: int):
    ...
    x_ = x.lexico()
    if 0 <= mu <= 3:
        x_ = np.einsum("tzyxab,tzyxib->tzyxia",
                       U_[mu], np.roll(x_, -1, 3 - mu))
    elif 4 <= mu <= 7:
        x_ = np.roll(np.einsum("tzyxba,tzyxib->tzyxia",
                     U_[mu - 4].conj(), x_), 1, 7 - mu)
...
for dim in range(Nd):
    for covdev_mu in range(8):
        ...
        gauge2.data[dim] = unit.covDev(
            x, covdev_mu).data[:, :, :, :, :, :Nc, :]
        gauge2.projectSU3(2e-15)
        shift(gauge, dim, covdev_mu)
        print(dim, covdev_mu, cp.linalg.norm(
            gauge.data[dim] - gauge2.data[dim]))
