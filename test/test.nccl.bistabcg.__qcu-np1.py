...
def compare(round):
    ...
    qcu_U.data[:] = U.data
    qcu_p.data[:] = p.data
    print(f'rank {rank} quda u and U difference: , {cp.linalg.norm(qcu_U.data - U.data) / cp.linalg.norm(qcu_U.data)}, takes {t2 - t1} sec, norm_qcu_U = {cp.linalg.norm(qcu_U.data)}')
    print(f'rank {rank} my p and p difference: , {cp.linalg.norm(qcu_p.data - p.data) / cp.linalg.norm(qcu_p.data)}, takes {t2 - t1} sec, my_p_norm = {cp.linalg.norm(qcu_p.data)}')
    print(f'rank {rank} my x and x difference: , {cp.linalg.norm(qcu_x.data - quda_x.data) / cp.linalg.norm(qcu_x.data)}, takes {t2 - t1} sec, my_x_norm = {cp.linalg.norm(qcu_x.data)}')
    if rank == 0:
        print('===============qcu==================')
    t1 = perf_counter()
    ...
    print("######quda:quda_x[-6,-6,-6,-6]:\n", quda_x.lexico()[-6, -6, -6, -6])
    print("######qcu:qcu_x[-6,-6,-6,-6]:\n", qcu_x.lexico()[-6, -6, -6, -6])
    print("######", qcu_x.lexico().shape)
    diff_x = np.abs((qcu_x.lexico()-quda_x.lexico()).real)
    diff = np.sum(diff_x, axis=(-1, -2))
    _ = np.where(diff > 1e-5)
    print("######", diff.shape)
    print("######", qcu_p.data.shape)
    print("######T:", _[0], ",\n", len(_[0]))
    print("######Z:", _[1], ",\n", len(_[1]))
    print("######Y:", _[2], ",\n", len(_[2]))
    print("######X:", _[3], ",\n", len(_[3]))
