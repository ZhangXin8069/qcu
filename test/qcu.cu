 ...
 */
__global__ void host_api_kernel(data_type *randomValues, data_type *out, int N)
{
    int i;
...
void use_host_api(int N)
{
    ...
    CHECK(cudaMalloc((void **)&dRand, sizeof(data_type) * N));
    CHECK(cudaMalloc((void **)&dOut, sizeof(data_type) * N));
    ...
    // Retrieve outputs
    CHECK(cudaMemcpy(hOut, dOut, sizeof(data_type) * N, cudaMemcpyDeviceToHost));
