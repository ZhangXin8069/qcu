...
template <typename OutputType, typename InputType, template <typename> class ReductionOp>
...
void reduction_gpu_async(OutputType* output, OutputType* temp, const InputType* input, int vector_length, cudaStream_t stream) {
    int block_size = 256;
...
void profile_reduction_gpu_sync(OutputType* output, OutputType* temp, const InputType* input, int vector_length, cudaStream_t stream) {
    int block_size = 256;
    int grid_size = (vector_length + block_size - 1) / block_size;
...
int main () {
    ...
    CHECK_CUDA(cudaMalloc(&d_input, size * sizeof(double)));
    CHECK_CUDA(cudaMalloc(&d_output, sizeof(double)));
    CHECK_CUDA(cudaMalloc(&d_temp, size * sizeof(double)));

    init_host_data(h_input.get(), size);
    CHECK_CUDA(cudaMemcpy(d_input, h_input.get(), size * sizeof(double), cudaMemcpyHostToDevice));
    
    // cpu
    double res;
    TIMER_EVENT(reduction_cpu(h_input.get(), size), 1, "reduction_cpu");
    // gpu
    // warm up
    profile_reduction_gpu_sync<double, double, AddOp>(d_output, d_temp, d_input, size, 0);
