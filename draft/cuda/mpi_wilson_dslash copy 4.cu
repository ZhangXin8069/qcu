...
<<<<<<< HEAD:src/cuda/wilson_dslash.cu
#ifdef WILSON_DSLASH
...
=======
>>>>>>> eed22d2 (2024-07-31-01-36-26):draft/cuda/mpi_wilson_dslash copy 4.cu
...
void mpiDslashQcu(void *fermion_out, void *fermion_in, void *gauge,
  ...
  if (grid_1dim[X] != 1) {
    ...
    wilson_dslash_x_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T],
        parity, recv_vec[B_X], recv_vec[F_X]);
  } else {
    checkCudaErrors(cudaDeviceSynchronize());
    wilson_dslash_x_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T],
        parity, send_vec[F_X], send_vec[B_X]);
  }
  // recv y
  if (grid_1dim[Y] != 1) {
    ...
    wilson_dslash_y_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T],
        parity, recv_vec[B_Y], recv_vec[F_Y]);
  } else {
    checkCudaErrors(cudaDeviceSynchronize());
    wilson_dslash_y_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T],
        parity, send_vec[F_Y], send_vec[B_Y]);
  }
  // recv z
  if (grid_1dim[Z] != 1) {
    ...
    wilson_dslash_z_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T],
        parity, recv_vec[B_Z], recv_vec[F_Z]);
  } else {
    checkCudaErrors(cudaDeviceSynchronize());
    wilson_dslash_z_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T],
        parity, send_vec[F_Z], send_vec[B_Z]);
  }
  // recv t
  if (grid_1dim[T] != 1) {
    ...
    wilson_dslash_t_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T],
        parity, recv_vec[B_T], recv_vec[F_T]);
  } else {
    checkCudaErrors(cudaDeviceSynchronize());
    wilson_dslash_t_recv<<<gridDim, blockDim>>>(
        gauge, fermion_out, lat_1dim[X], lat_1dim[Y], lat_1dim[Z], lat_1dim[T],
        parity, send_vec[F_T], send_vec[B_T]);
  }
