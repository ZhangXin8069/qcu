...
struct LatticeWilsonDslash {
  ...
  void run(void *fermion_out, void *fermion_in, void *gauge, int parity) {
    {
      checkCudaErrors(cudaStreamSynchronize(set_ptr->qcu_stream)); // needed
    ...
    }
    {
      ...
      checkCudaErrors(cudaStreamSynchronize(set_ptr->qcu_streams[_F_X_]));
      checkCudaErrors(cudaStreamSynchronize(set_ptr->qcu_streams[_F_Y_]));
      checkCudaErrors(cudaStreamSynchronize(set_ptr->qcu_streams[_F_Z_]));
      checkCudaErrors(cudaStreamSynchronize(set_ptr->qcu_streams[_F_T_]));
    }
  }
