...
struct LatticeBistabcg {
  ...
  void _dslash(void *fermion_out, void *fermion_in, void *gauge,
    ...
    dslash.run_eo(device_vec0, fermion_in, gauge, stream_index);
    dslash.run_oe(device_vec1, device_vec0, gauge, stream_index);
    bistabcg_give_dest_o<<<set_ptr->gridDim, set_ptr->blockDim, 0,
                           set_ptr->streams[stream_index]>>>(
        fermion_out, fermion_in, device_vec1, _KAPPA_);
  }
  void init(void *_b_e, void *_b_o, void *gauge) {
    b_e = _b_e;
    b_o = _b_o;
    dslash.run_oe(device_vec0, b_e, gauge, _a_);
  ...
  void init(void *gauge) {
    ...
    checkCudaErrors(cudaStreamSynchronize(set_ptr->stream));
    checkCudaErrors(cudaStreamSynchronize(set_ptr->stream));
    dslash.run_eo(device_vec0, ans_o, gauge,_a_);
    checkCudaErrors(cudaStreamSynchronize(set_ptr->streams[_a_]));
    ...
    dslash.run_oe(device_vec1, ans_e, gauge,_a_);
    checkCudaErrors(cudaStreamSynchronize(set_ptr->streams[_a_]));
