  ...
  struct LatticeGmresIr
  {
    ...
    void __init()
    {
      ...
      if (if_input == 0)
      {
        ...
        gmres_ir_give_b_e<T><<<set_ptr->gridDim, set_ptr->blockDim, 0,
                               set_ptr->stream>>>(b_e, ans_e, device_vec0, set_ptr->kappa(),
                                                  device_vals);
        wilson_dslash.run_oe(device_vec1, ans_e, gauge);
        gmres_ir_give_b_o<T><<<set_ptr->gridDim, set_ptr->blockDim, 0,
      ...
      { // give b__o, x_o, rr
        ...
        gmres_ir_give_b__o<T><<<set_ptr->gridDim, set_ptr->blockDim, 0,
        ...
        gmres_ir_give_rr<T><<<set_ptr->gridDim, set_ptr->blockDim, 0,
    ...
    void _wilson_dslash(void *fermion_out, void *fermion_in, void *gauge)
    {
      ...
      gmres_ir_give_dest_o<T><<<set_ptr->gridDim, set_ptr->blockDim, 0,
                                set_ptr->stream>>>(
    ...
    { // there is a bug
      ...
      gmres_ir_give_diff<T><<<set_ptr->gridDim, set_ptr->blockDim, 0,
                              set_ptr->streams[_a_]>>>(x, ans, device_vec0,
                                                       device_vals);
      _dot(device_vec0, device_vec0, _diff_tmp_, _a_);
      gmres_ir_give_1diff<T><<<1, 1, 0, set_ptr->streams[_a_]>>>(device_vals);
      print_vals(999);
      checkCudaErrors(cudaStreamSynchronize(set_ptr->stream));
      checkCudaErrors(cudaStreamSynchronize(set_ptr->streams[_a_]));
      checkCudaErrors(cudaStreamSynchronize(set_ptr->streams[_b_]));
    ...
    void run()
    {
      ...
      for (int loop = 0; loop < _MAX_ITER_; loop++)
      {
        ...
        checkCudaErrors(cudaStreamSynchronize(set_ptr->streams[_b_]));
        {
          ...
          gmres_ir_give_1beta<T><<<1, 1, 0, set_ptr->streams[_a_]>>>(device_vals);
        }
        checkCudaErrors(cudaStreamSynchronize(
            set_ptr->streams[_a_])); // needed, but don't know why.
        {
          // rho_prev = rho;
          gmres_ir_give_1rho_prev<T><<<1, 1, 0, set_ptr->streams[_b_]>>>(
              device_vals);
        }
        {
          // p[i] = r[i] + (p[i] - v[i] * omega) * beta;
          gmres_ir_give_p<T><<<set_ptr->gridDim, set_ptr->blockDim, 0,
        ...
        {
          // alpha = rho / tmp0;
          gmres_ir_give_1alpha<T><<<1, 1, 0, set_ptr->streams[_d_]>>>(device_vals);
        }
        checkCudaErrors(cudaStreamSynchronize(set_ptr->streams[_d_]));
        {
          // s[i] = r[i] - v[i] * alpha;
          gmres_ir_give_s<T><<<set_ptr->gridDim, set_ptr->blockDim, 0,
        ...
        checkCudaErrors(cudaStreamSynchronize(set_ptr->streams[_c_]));
        {
          // break;
          checkCudaErrors(cudaMemcpyAsync(
              ((static_cast<LatticeComplex<T> *>(host_vals)) + _norm2_tmp_),
              ((static_cast<LatticeComplex<T> *>(device_vals)) + _norm2_tmp_),
              sizeof(LatticeComplex<T>), cudaMemcpyDeviceToHost,
              set_ptr->streams[_d_]));
        }
        {
          // omega = tmp0 / tmp1;
          gmres_ir_give_1omega<T><<<1, 1, 0, set_ptr->streams[_d_]>>>(device_vals);
        }
        checkCudaErrors(cudaStreamSynchronize(set_ptr->streams[_d_]));
        {
          // r[i] = s[i] - t[i] * omega;
        ...
        }
        {
          ...
          gmres_ir_give_x_o<T><<<set_ptr->gridDim, set_ptr->blockDim, 0,
