...
public:
  ...
  __host__ __device__ LatticeParam(int lat_x, int lat_y, int lat_z, int lat_t, int grid_x, int grid_y, int grid_z, int grid_t, int parity):_parity(parity) {
    _lat_size[X] = lat_x;
    _lat_size[Y] = lat_y;
    _lat_size[Z] = lat_z;
    _lat_size[T] = lat_t;
  ...
  __host__ __device__ LatticeParam(int *lat_ptr, int *grid_ptr, int parity):_parity(parity) {
  ...
  // Get lattice
  __host__ __device__ int lat(int dim) const {
    return _lat_size[dim];
  }
  ...
  // Get grid
  __host__ __device__ int grid(int dim) const {
